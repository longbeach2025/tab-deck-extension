...
function usage() {
  return [
    "Usage:",
    "  SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/import_init_bundle_to_supabase.mjs \\",
    "    --input supabase/init/alpha27/tab-deck-alpha27-init-bundle.json.gz --user-id <auth_user_uuid> [--batch-size 200]",
    "",
    "Options:",
...
async function main() {
  ...
  const raw = await readMaybeGzip(args.input);
  ...
  console.log(
    `[import-init] spaces=${spaces.length} collections=${collections.length} links=${links.length} batchSize=${args.batchSize}`
  );
  ...
  if (args.setActiveSpace) {
    const activeSpaceId = bundle.meta?.activeSpaceId || spaces[0].id;
    const payload = {
      user_id: userId,
  ...
  console.log(`[done] init bundle import completed in ${elapsedMs} ms`);
}
